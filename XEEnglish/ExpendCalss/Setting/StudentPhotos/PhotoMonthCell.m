...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        ...
        self.button1 = [PhotoMonthButton buttonWithType:UIButtonTypeCustom];
        [self.button1 setFrame:CGRectMake(0, 0, 90, 140)];
        self.button1.tag = 1;
        [self.button1 addTarget:self action:@selector(buttonPressed:) forControlEvents:UIControlEventTouchUpInside];
        [self.contentView addSubview:self.button1];
        
        self.button2 =[PhotoMonthButton buttonWithType:UIButtonTypeCustom];
        [self.button2 setFrame:CGRectMake(100, 0, 90, 140)];
        self.button2.tag = 2;
        [self.button2 addTarget:self action:@selector(buttonPressed:) forControlEvents:UIControlEventTouchUpInside];
        [self.contentView addSubview:self.button2];
        
        self.button3 =[PhotoMonthButton buttonWithType:UIButtonTypeCustom];
        [self.button3 setFrame:CGRectMake(200, 0, 90, 140)];
        self.button3.tag = 3;
...
- (IBAction)buttonPressed:(id)sender{
    ...
    if (button.tag == 1) {
        [self.delegate PhotoMonthCellButtonPressed:sender andServiceInfo:_serviceDic1];
    }
    else if (button.tag == 2){
        [self.delegate PhotoMonthCellButtonPressed:sender andServiceInfo:_serviceDic2];
    }
    else if (button.tag == 3){
        [self.delegate PhotoMonthCellButtonPressed:sender andServiceInfo:_serviceDic3];
    }
