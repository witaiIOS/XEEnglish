...
- (NSString *)changeUnitForTotalFee:(NSString *)totalFee {
    CGFloat fee = totalFee.floatValue;
    
    return [NSString stringWithFormat:@"%.0f",fee*100];
}
...
// 负责人：李启波（marcyli）
//============================================================
- ( NSMutableDictionary *)sendPayWithBody:(NSString *)body andOrderNo:(NSString *)orderno andOrderPrice:(NSString *)orderPrice
{
    ...
    [packageParams setObject: kWxNOTIFY_URL     forKey:@"notify_url"];  //支付结果异步通知
    [packageParams setObject: orderno           forKey:@"out_trade_no"];//商户订单号
    [packageParams setObject: @"196.168.0.1"    forKey:@"spbill_create_ip"];//发器支付的机器ip
    [packageParams setObject: [self changeUnitForTotalFee:orderPrice]       forKey:@"total_fee"];       //订单金额，单位为分
